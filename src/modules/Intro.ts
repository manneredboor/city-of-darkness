import 'modules/vendor/numeric.js'
import { Vector, vec } from 'modules/utils/Vector'
import initialState from 'modules/utils/initialState'
import { getTransform } from 'modules/utils/matrixTransform'
import bgMask from 'modules/utils/mask'
...
import { minmax, rnd } from 'modules/utils/math'
...
export class Intro {
  ...
  constructor() {
    ...
    this.canvas.classList.add('kwc-intro__canvas')

    this.intoBody = document.createElement('div')
    this.intoBody.classList.add('kwc-intro__body')

    const windowLights = document.createElement('div') as HTMLDivElement
    windowLights.classList.add('kwc-intro__lights')
    ...
    intro.appendChild(windowLights)
    ...
    //   this.state.mouse = vec(e.pageX, e.pageY)
    //   bgMask.push(this.saveVec({ x: e.pageX, y: e.pageY }))
    //   console.log(JSON.stringify(bgMask))
    // })
  ...
  renderDarkness(time: number, now: number) {
    ...
    const n = noise.simplex3(0, 0, (time % 10000) / 300)
    const hlRadius = Math.max(w / 6, h / 6)
    ...
    const lightX = mouse.x
    const lightY = mouse.y
    ...
    ctx.globalAlpha = minmax(0, 1 - n * 0.6, 1)
    ctx.fillStyle = g
  ...
  renderLetters = () => {
    ...
    this.state.textDrawings.forEach((itm, i) => {
      ...
      const textW = measureText(
        itm.text,
        'kwc-intro__text' + (itm.isChinese ? ' kwc-intro__text-chinese' : ''),
        fz,
      )
      ...
      stopBoxes.forEach(
        ({ x1, y1, x2, y2, nx1, ny1, nx2, ny2, p1, np1, p2, np2 }, j) => {
          ...
          perspective.classList.add('kwc-intro__text-perspective')
          ...
          text.classList.add('kwc-intro__text')
          if (itm.isChinese) text.classList.add('kwc-intro__text-chinese')
          ...
          textMover.classList.add('kwc-intro__text-mover')
